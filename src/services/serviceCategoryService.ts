...
const normalizeCategory = (raw: any): ExperienceCategory => {
  ...
  return {
    ...
    isActive:
      (raw?.isActive ??
        raw?.IsActive ??
        (statusRaw ? statusRaw === "ACTIVE" : true)) === true,
  };
};
  ...
  async list(): Promise<ExperienceCategory[]> {
    try {
      const res = await apiService.get<any>(
        apiConfig.endpoints.serviceCategory.list,
      );
  ...
  async create(
    ...
    try {
      ...
      const res = await apiService.post<any>(
        apiConfig.endpoints.serviceCategory.create,
        body,
      );
      ...
      return {
        success: false,
        message:
          error instanceof Error ? error.message : "Khong the tao danh muc",
      };
    }
  },
  ...
  async update(
    ...
    try {
      ...
      const res = await apiService.put<any>(
        apiConfig.endpoints.serviceCategory.update(id),
        body,
      );
      const item = res?.data ?? res;
      return {
  ...
  async remove(id: string): Promise<{ success: boolean; message?: string }> {
    try {
      const res = await apiService.delete<any>(
        apiConfig.endpoints.serviceCategory.delete(id),
      );
      return {
        success: res?.success ?? true,
      ...
      return {
        success: false,
        message:
          error instanceof Error ? error.message : "Khong the xoa danh muc",
      };
    }
  },
};
