...
const mapBooking = (item: any): Booking => {
  ...
  return {
    ...
    groupBookingId: get("groupBookingId", "GroupBookingId") ?? null,
    ...
    checkInTime: String(
      get("checkInTime", "CheckInTime") ??
        get("checkIn_time", "CheckIn_time") ??
        "",
    ),
    checkOutTime: String(
      get("checkOutTime", "CheckOutTime") ??
    ...
    bookedExperiences: Array.isArray(
      ...
      ? (get("bookedExperiences", "BookedExperiences") as any[]).map(
          (be: any) => ({
            id: String(be?.id ?? ""),
...
export const bookingService = {
  ...
  async cancelBooking(
    ...
    try {
      ...
      return {
        success: response?.success ?? true,
        message: response?.message ?? "Da huy don dat phong thanh cong.",
      };
    } catch (error) {
  ...
  async modifyBooking(
    ...
    try {
      ...
      return {
        success: response?.success ?? true,
        message: response?.message ?? "Cap nhat booking thanh cong!",
      };
    } catch (error) {
  ...
  async addExperiencesViaModify(
    ...
    try {
      ...
      return {
        success: false,
        message:
          error instanceof Error ? error.message : "Loi khi them dich vu",
      };
    }
  },
  ...
  async updateSpecialRequests(
    ...
    try {
      ...
      return {
        success: response?.success ?? true,
        message: response?.message ?? "Da ghi nhan yeu cau dac biet.",
      };
    } catch (error) {
      ...
      return {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Da xay ra loi khi cap nhat yeu cau dac biet",
      };
    }
  },
  ...
  async previewRefund(
    id: string,
  ): Promise<{ estimatedRefund: number; message: string } | null> {
    try {
      const response = await apiService.get<any>(
  ...
  async cancelAndRefund(
    ...
    try {
      const res = await fetch(
        `${apiConfig.baseURL}${apiConfig.endpoints.bookings.cancelAndRefund}`,
        {
          method: "POST",
          headers: {
      ...
      const isSuccess: boolean =
        data?.isSuccess ?? data?.IsSuccess ?? body?.success ?? res.ok;
      ...
      const message: string =
        ...
        data?.Message ??
        body?.message ??
        (res.ok ? "Huy phong thanh cong." : `Loi ${res.status}`);
