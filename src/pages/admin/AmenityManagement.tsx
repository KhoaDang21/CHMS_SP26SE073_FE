...
export default function AmenityManagement() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast.error('Kh�ng th? t?i d? li?u ti?n �ch');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async () => {
    if (!formData.name.trim()) {
      toast.error('Vui l�ng di?n t�n ti?n �ch');
      return;
    }
    ...
    try {
      ...
      if (result?.success) {
        toast.success('Th�m ti?n �ch th�nh c�ng');
        setShowModal(false);
        resetForm();
        loadData();
      } else {
        toast.error(result?.message || 'Kh�ng th? th�m ti?n �ch');
      }
    } catch (error) {
      console.error('Error creating amenity:', error);
      toast.error('Kh�ng th? th�m ti?n �ch. Vui l�ng ki?m tra d? li?u nh?p.');
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (result?.success) {
        toast.success('C?p nh?t ti?n �ch th�nh c�ng');
        setShowModal(false);
        setEditingAmenity(null);
        resetForm();
        loadData();
      } else {
        toast.error(result?.message || 'Kh�ng th? c?p nh?t ti?n �ch');
      }
    } catch (error) {
      console.error('Error updating amenity:', error);
      toast.error('Kh�ng th? c?p nh?t ti?n �ch');
    }
  };

  const handleDelete = async (id: string) => {
    if (!confirm('B?n c� ch?c mu?n x�a ti?n �ch n�y?')) return;

    try {
      const result = await adminAmenityService.deleteAmenity(id);
      if (result?.success) {
        toast.success('X�a ti?n �ch th�nh c�ng');
        loadData();
      } else {
        toast.error(result?.message || 'Kh�ng th? x�a ti?n �ch');
      }
    } catch (error) {
      console.error('Error deleting amenity:', error);
      toast.error('Kh�ng th? x�a ti?n �ch');
    }
  };
  ...
  const handleToggleStatus = async (amenity: Amenity) => {
    try {
      ...
      toast.error('Kh�ng th? c?p nh?t tr?ng th�i');
    }
  };
